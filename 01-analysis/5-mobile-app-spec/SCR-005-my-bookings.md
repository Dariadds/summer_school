...
**Зона авторизации:** АЗ  
**Дизайн-макет:** — (ASCII-макет из дизайн-брифа)
...
Данные загружаются через `listBookings` (`GET /bookings`) и содержат только брони текущего клиента (NFR-12). Разделение на предстоящие/прошедшие и признак «прошедшее» **вычисляются на клиенте** по `slot.start_at` и `status` — отдельного флага сервер не отдаёт.
...
|--------|-----------------|----------|
| [LOGIC-003 Расчёт цены брони](09_Логики/LOGIC-003_Расчёт-цены-брони.md) | Карточка записи, поле итоговой цены | Расчёт итога по брони `price × seats_count + rental_price × rental_count` (RUB). |
...
|----------|-----|----------------|----------|----------|
| `status` | string (`active`/`cancelled`/`late_cancel`) | Нет | — | Опциональный фильтр по статусу. На SCR-005 **не передаётся**: грузим все брони, разделение делаем на клиенте. |
| `limit` | integer | Нет | — | Размер страницы. В MVP история грузится целиком, лимит не хардкодим. |
| `offset` | integer | Нет | — | Смещение для пагинации (при необходимости догрузки). |
...
**Ответ (200):** `BookingListResponse` = `items: BookingSummary[]` + `meta` (пагинация). Поля `BookingSummary`, используемые на экране: `id`, `seats_count`, `rental_count`, `price`, `rental_price`, `status`, `cancelled_at`, `slot` (`SlotSummary` → `start_at`, `route.name`, `route.type`, `instructor.name`).
...
- **Прошедшие** — `slot.start_at <= now` **ИЛИ** `status ∈ {cancelled, late_cancel}` (отменённая запись с будущим стартом тоже попадает в «Прошедшие»).
...
> Признак «прошедшая» — производный (вычисляется из `slot.start_at`), а не статус брони. Параметр `status` запроса в API **не передаётся** (см. [listBookings](#listbookings)); разделение на табы выполняется на клиенте по `slot.start_at` и `Booking.status`.
...
| Итог цены | Итоговая цена по записи, RUB; оплата офлайн | вычисляется из `price`, `rental_price`, `seats_count`, `rental_count` (см. [LOGIC-003](09_Логики/LOGIC-003_Расчёт-цены-брони.md)) | — | — |
| Бейдж статуса | Текст + форма: `active`→«активна», `cancelled`→«отменена», `late_cancel`→«поздняя отмена» | `status` | — | — |
...
**Логика:**
- Итог цены: [LOGIC-003](09_Логики/LOGIC-003_Расчёт-цены-брони.md) — итог вычисляется по формуле `price × seats_count + rental_price × rental_count` из полей брони и её слота; отдельного поля `price_total` API не возвращает.
...
| Обновить список | Список | Pull-to-refresh | Повторный запрос [listBookings](#listbookings). Успех — обновлённый контент **без снека**; ошибка — список сохраняется + снек «Не удалось обновить. Проверьте соединение и попробуйте снова.» |
...
| AC-004 | **Дано** бронь имеет статус `active`/`cancelled`/`late_cancel`, **Когда** клиент видит карточку, **Тогда** на ней бейдж «активна»/«отменена»/«поздняя отмена», переданный текстом и формой, а не только цветом | P0 |
...
| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** бронь отменена (`cancelled`/`late_cancel`), но `slot.start_at` ещё в будущем, **Когда** клиент открывает экран, **Тогда** она показана в «Прошедших», а не в «Предстоящих» | P0 |
