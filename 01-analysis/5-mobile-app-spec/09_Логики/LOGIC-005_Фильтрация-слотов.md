...
| `filters.date_from` | Состояние | `date-time` или не задано | Начало периода старта. По умолчанию — не задано (все предстоящие). |
| `filters.date_to` | Состояние | `date-time` или не задано | Конец периода старта. По умолчанию — не задано. |
...
только как `true`, дефолтное `false` опускается (заполненные слоты приходят в выдаче).
...
|----------|-----|----------|-------------------|
| `date_from` | string (date-time) | Начало периода старта | `appliedFilters.date_from`; опускается, если не задано (дефолт — все предстоящие) |
| `date_to` | string (date-time) | Конец периода старта | `appliedFilters.date_to`; опускается, если не задано |
...
| AC-005 | **Дано** заданы какие-либо фильтры, **Когда** клиент нажимает «Сбросить», а затем «Применить», **Тогда** все параметры возвращаются к дефолтам, SCR-002 показывает все предстоящие слоты, индикатор активных фильтров снят. |
