...
- **Удаление аккаунта** (деструктивно, ПДн) → `deleteAccount` (DELETE `/profile`) с обязательным явным подтверждением → [SCR-001](SCR-001-registration.md).
...
- Заголовок «Удалить аккаунт?»; пояснение «Ваши данные и записи будут удалены безвозвратно. Это действие нельзя отменить.»
...
| AC-006 | **Дано** клиент на экране «Профиль», **Когда** он нажимает «Удалить аккаунт» и подтверждает деструктивное действие, **Тогда** отправляется `deleteAccount`, аккаунт и данные удаляются, сессия завершается и он попадает на [SCR-001](SCR-001-registration.md). | P0 |
