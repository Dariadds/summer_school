...
расписания. По умолчанию (`only_available=false`) показываются **все** предстоящие слоты,
отсортированные по `start_at` по возрастанию (ближайшие первыми) и сгруппированные по дням
со sticky-заголовками. Каждая карточка слота содержит дату/время старта, название и тип
...
| `appliedFilters.date_from` | Состояние | `date-time` или не задано | Начало периода старта; по умолчанию не задано (все предстоящие). |
| `appliedFilters.date_to` | Состояние | `date-time` или не задано | Конец периода старта; по умолчанию не задано. |
...
|----------|-----|----------------|----------|----------|
| `date_from` | string (date-time) | Нет | `appliedFilters.date_from` | Начало периода старта; опускается, если не задано (дефолт — все предстоящие). |
| `date_to` | string (date-time) | Нет | `appliedFilters.date_to` | Конец периода старта; опускается, если не задано. |
...
| Успех | `items` пуст, фильтры в дефолте | Empty «нет слотов вообще»: заголовок «Пока нет доступных прогулок» + подсказка «Потяните вниз, чтобы обновить» (тексты — каталог Empty в [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md) Шаг 3) |
...
| Заглушка Empty «нет слотов вообще» | Заголовок «Пока нет доступных прогулок» + подсказка «Потяните вниз, чтобы обновить» (каталог Empty [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md) Шаг 3) | Условно (при пустом `items` и фильтрах в дефолте) |
...
| Подсказка Empty «нет слотов вообще» | «Потяните вниз, чтобы обновить» | — | — | — (обновление — жестом pull-to-refresh) |
...
| Empty (нет слотов вообще) | 200 + `items` пуст, фильтры в дефолте | Заглушка: «Пока нет доступных прогулок» + «Потяните вниз, чтобы обновить» (каталог Empty [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md) Шаг 3) |
...
stateDiagram-v2
    ...
    EmptyNoSlots --> Loading : pull-to-refresh («Потяните вниз, чтобы обновить»)
...
|----|----------|-----------|
| AC-001 | **Дано** клиент авторизован и открыл таб «Прогулки», фильтры в дефолте, **Когда** `listSlots` вернул непустой список, **Тогда** показаны все предстоящие слоты, каждая карточка содержит дату/время старта, маршрут и тип, инструктора, цену и «Свободно N из M». | P0 |
...
| ID | Критерий | Приоритет |
|----|----------|-----------|
| AC-E01 | **Дано** в системе нет ни одного предстоящего слота и фильтры в дефолте, **Когда** `listSlots` вернул пустой `items`, **Тогда** показан empty state «Пока нет доступных прогулок» с подсказкой «Потяните вниз, чтобы обновить» (без CTA «Изменить фильтры»; каталог Empty [LOGIC-008](09_Логики/LOGIC-008_Паттерн-состояний-экрана.md) Шаг 3). | P1 |
