...
**Зона авторизации:** АЗ  
**Дизайн-макет:** — (бренд-макет не зафиксирован; ASCII-вайрфрейм — из [дизайн-брифа SCR-006](../3-design-brief/SCR-006-booking-details.md))
...
|--------|-----------------|----------|
| [LOGIC-003 Расчёт цены брони](09_Логики/LOGIC-003_Расчёт-цены-брони.md) | Блок «Цена» | Расчёт итоговой суммы `price × seats_count + rental_price × rental_count`; разбивка из данных брони/слота. |
...
| Бейдж статуса | Иконка + текст статуса (Активна / Отменена / Поздняя отмена / Прошедшая). | Да |
...
| Служебные метки | Дата создания; дата отмены — только если бронь отменена. | Да (создана) / Опционально (отменена) |
...
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Итоговая цена | Сумма к оплате, ₽ (RUB) | вычисляется из `price`, `rental_price`, `seats_count`, `rental_count` (из [getBooking](#getbooking)) по [LOGIC-003](09_Логики/LOGIC-003_Расчёт-цены-брони.md) | — | — |
...
**Логика:**
- Цена: [LOGIC-003](09_Логики/LOGIC-003_Расчёт-цены-брони.md). Итог — производная величина, API отдельного поля `price_total` не возвращает; вычисляется на клиенте по [LOGIC-003](09_Логики/LOGIC-003_Расчёт-цены-брони.md) из полей брони (`seats_count`, `rental_count`) и её слота (`price`, `rental_price`).
...
**Логика:**
- Метка «Отменено» отображается **только** если `cancelled_at != null` (для статусов `cancelled` / `late_cancel`).
...
- Кнопка **неактивна (disabled, не скрывается)** с обязательным пояснением, если:
  ...
  - `status = cancelled` или `status = late_cancel` (UC-2 E2, повторная отмена не выполняется) → «Запись уже отменена.»
...
stateDiagram-v2
    ...
    Loading --> Cancelled : 200 OK (cancelled / late_cancel)
...
| AC-005 | **Дано** бронь со статусом `cancelled` или `late_cancel`, **Когда** экран в Content, **Тогда** отображается дата отмены (`cancelled_at`) | P0 |
...
| AC-N03 | **Дано** бронь уже отменена (`cancelled` / `late_cancel`, UC-2 E2), **Когда** экран в Content, **Тогда** кнопка «Отменить» disabled с пояснением «Запись уже отменена» (повторная отмена недоступна) | P0 |
