...
> подставляются из данных слота/маршрута. Доступные места считаются как
> `min(свободные прокатные доски, потолок маршрута)`.
...
- Контраст и размеры — см. §3.2, §3.1.
- Поддержка системного увеличения шрифта; layout не «ломается» при крупном тексте.
- Все интерактивные элементы имеют текстовую подпись/доступное имя (screen reader).
- Фокус-состояния и обратная связь на тап обязательны.
- Не полагаться на цвет как единственный носитель смысла (§3.2).
